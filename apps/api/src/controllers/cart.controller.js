      ...
      const result = await carts.update(
        {
          ...
          total_price:
            findCart.total_price + req.body.price * req.body.quantity,
          total_weight:
            findCart.total_weight + req.body.weight * req.body.quantity,
          updatedAt: new Date(),
        },
        {
          where: {
  ...
  try {
      ...
      include: [
        {
          ...
          include: [
            { model: product_images, required: true, attributes: ['image'] },
          ],
        },
      ],
      raw: true,
    ...
    allProduct.rows.map((val, id) => {
      if (!index.includes(allProduct.rows[id].product_id)) {
        ...
        result.push({
          ...allProduct.rows[id],
          productWeightConvert: val[`product.weight`] / 1000,
          total_weightConvert: val.total_weight / 1000,
          isChecked: false,
        });
      }
    });
    ...
    stock.map((val, idx) => {
      ...
      let id = stockProduct.findIndex(
        (val) => val.product_id == filterProductId,
      );
      if (id < 0) {
    ...
    stockProduct.map((val, id) => {
      let idx = result.findIndex(
        (val) => val.product_id == stockProduct[id].product_id,
      );
      if (idx >= 0) {
...
export const plusQty = async (req, res, next) => {
  try {
    const findProduct = await carts.findOne({
      where: {
        [Op.and]: [{ account_id: req.userData.id }, { id: req.params.id }],
      },
      include: [
        {
          model: products,
      ...
      const data = await carts.update(
        {
          ...
          total_weight:
            findProduct.total_weight + findProduct[`product.weight`],
        },
        {
          where: {
            id: findProduct.id,
