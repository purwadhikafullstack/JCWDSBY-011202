...
export const getCartToCheckout = async (req, res, next) => {
  try {
    ...
    let checkoutPrice = allPrice.reduce((a, b) => {
      return a + b;
    });
    let checkoutWeight = allWeight.reduce((a, b) => {
      return a + b;
    });
...
export const getUserData = async (req, res, next) => {
  try {
    const result = await accounts.findOne({
      ...
      attributes: {
        exclude: ['password', 'createdAt', 'updatedAt', 'is_deleted', 'id'],
      },
      // include: [{
      //     model: addresses,
      //     required: true,
...
export const getShippingCost = async (req, res, next) => {
  try {
    ...
    const originId = allCity.findIndex(
      (val) => val.city_name == origin[0]['city.name'],
    );
    const destinationId = allCity.findIndex(
      (val) => val.city_name == req.query.kota,
    );
