...
export const createNewAddress = async (req, res, next) => {
  try {
    ...
    const searchLatLon = await axios.get(
      `https://api.opencagedata.com/geocode/v1/json?q=${req.body.address}%2C+${cityName.name}%2C+Indonesia&key=4f33f6aa35b34fca97c8c6d583fbe834`,
    );
    ...
    return res
      .status(200)
      .send({ success: true, message: 'SUCCESS ADD ADDRESS' });
  } catch (error) {
    return res
      .status(500)
      .send({ success: false, message: 'ERROR CREATE DATA' });
  }
};
export const deleteAddress = async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .send({ success: true, message: 'Success delete adress' });
  } catch (error) {
    return res.status(500).send(error);
  }
};
...
export const editAddress = async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .send({ success: true, message: 'Success edit address' });
  } catch (error) {
    return res.status(500).send(error);
  }
};
