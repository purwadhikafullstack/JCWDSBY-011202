...
export const requestMutation = async (req, res, next) => {
    try {
        ...
        const productStock = await warehouse_storage.findAll({
            where: {
                warehouse_id: orderItems.warehouse_id,
                ...
                for (let j = 0; j < searchStock.length; j++) {
                    if (
                        ...
                        const countMutation = await warehouse_mutation.findAndCountAll({
                            where: {
                                destination_warehouse_id: orderItems.warehouse_id
                            }, raw: true
                        })
                        ...
                        itemMutation.push({
                            mutation_code: `MUT/${date.getFullYear()}${date.getMonth()}${date.getDate()}/ORI${orderItems.warehouse_id}/DEST${searchStock[j].warehouse_id}/PR${productLessStock[i].product_id}/${countMutation.count}/${j + 1}`,
